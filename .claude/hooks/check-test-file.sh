...
# component, page, router, hook, or util is created or edited, per
# CLAUDE.md's folder structure + testing convention.
...
if echo "$path" | grep -qE 'src/(components|pages|routes)/.*\.(tsx|jsx)$'; then
