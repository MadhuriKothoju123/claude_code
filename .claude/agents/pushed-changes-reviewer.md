...
4. `mcp__github__search_pull_requests` with `query: "repo:claude_code
   ...
   branch.
   - **If a PR exists**: get its number, then follow the same review
     protocol as the `pr-reviewer` agent — `pull_request_read` (`get_diff`,
...
5. Regardless of path, check conventions for whatever the diff touches:
   - Root `CLAUDE.md` — strict TypeScript, `npm run verify` workflow,
     server/client boundaries.
   - `client/CLAUDE.md` — function components/hooks only, and the
     matching-Jest-test-file rule for anything under `client/src/components`.
   - If the diff touches `client/src/components/**`, also check against the
     `react-mui-component` skill's rules (MUI-only, no inline styles,
     `Props` interface, ~150 line budget, one component per file).
